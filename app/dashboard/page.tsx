...
import { Calendar, MapPin, Clock, ArrowRight, Settings, CreditCard, Heart, History, LogOut, Users, IndianRupee, TrendingUp, Star } from "lucide-react";
...
function TravelerDashboard({ bookings, userName }: { bookings: BookingRow[], userName: string }) {
  const tabs = [
    { name: "Upcoming Trips", icon: Calendar, active: true },
                ...
                <div className="bg-brand-blue/5 p-6 border-b">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <h2 className="text-xl font-bold">Trip with {booking.other_person?.full_name || 'Guide'}</h2>
                    </div>
...
function GuideDashboard({ bookings, userName, userId }: { bookings: BookingRow[], userName: string, userId: string }) {
  const tabs = [
    { name: "My Tours", icon: MapPin, active: true },
    { name: "Earnings", icon: IndianRupee, active: false },
    { name: "Reviews", icon: Star, active: false },
    { name: "Profile Settings", icon: Settings, active: false },
  ];
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[250px_1fr]">
      ...
      <div className="space-y-6">
        ...
        {bookings.length === 0 ? (
          <div className="rounded-3xl border bg-card p-10 text-center shadow-sm">
            <h3 className="text-xl font-bold mb-2">No upcoming tours</h3>
              ...
              <div className="rounded-3xl border bg-card overflow-hidden shadow-sm">
                <div className="bg-brand-blue/5 p-6 border-b">
                  <div className="flex items-center justify-between">
                    <div>
                      <Badge className="bg-brand-emerald text-white hover:bg-emerald-600 mb-2 capitalize">{booking.status}</Badge>
                      <h2 className="text-xl font-bold">Tour booked by {booking.other_person?.full_name || 'Traveler'}</h2>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-muted-foreground">Booking Ref</p>
                      <p className="font-mono font-medium text-xs">#{booking.id.split('-')[0]}</p>
                ...
                <div className="p-6">
                  <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 mb-6">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-brand-emerald">{formatPrice(booking.total_price)}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-6 border-t">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm text-muted-foreground">Your Traveler</p>
                        <p className="font-semibold">{booking.other_person?.full_name}</p>
                      </div>
                    </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <ScrollReveal>
          <div className="mb-8">
            ...
            <p className="text-muted-foreground mt-1">Welcome back, {userName}! You have {bookings.length} upcoming {isGuide ? 'tours' : 'trips'}.</p>
          </div>
        </ScrollReveal>
